...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🤖 *Binance P2P Bot*\n\nP2P orders monitor လုပ်ပြီး market ထက် နှိမ့်တဲ့ order တွေ alert ပေးတယ်။",
        parse_mode="Markdown",
...
async def button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "back":
        u["waiting"] = None
        await q.edit_message_text("🤖 *P2P Bot*\nMenu ရွေးပါ:", parse_mode="Markdown", reply_markup=main_kb())
    elif d == "settings":
        txt = (f"⚙️ *Settings*\n\n🪙 Coin: `{u['coin']}`\n💰 Amount: `{u['amount']} USDT`\n"
        ...
        await q.edit_message_text(f"📈 *{u['coin']}/MMK*\n\n`{price:,.0f} MMK`",
                                   parse_mode="Markdown", reply_markup=main_kb())
    elif d == "start_bot":
        u["running"] = True
...
async def text_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if w == "coin":
        ...
        try:
            ...
            u["waiting"] = None
            await update.message.reply_text(f"✅ Min: {u['min_price']:,.0f} MMK", reply_markup=main_kb())
        except:
        ...
        try:
            ...
            u["waiting"] = None
            await update.message.reply_text(f"✅ Max: {u['max_price']:,.0f} MMK", reply_markup=main_kb())
        except:
            await update.message.reply_text("❌ ဂဏန်းသာ ထည့်ပါ!")
    elif w == "amount":
        try:
        ...
        try:
            ...
            for order in orders:
                ...
                if (price < market * 0.998 and
                    ...
                    msg = (f"🚨 *Low Price Order!*\n\n👤 `{nick}`\n"
                           f"💵 Price: `{price:,.0f} MMK`\n📈 Market: `{market:,.0f} MMK`\n"
                           f"📉 Below: `{diff:.2f}%`\n💰 `{min_amt:.0f} - {max_amt:.0f} USDT`\n"
